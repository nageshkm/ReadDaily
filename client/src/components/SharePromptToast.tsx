...
export function SharePromptToast({ isVisible, onDismiss }: SharePromptToastProps) {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-50 animate-in slide-in-from-bottom-2 duration-300">
      <div className="max-w-md mx-auto bg-background border border-border rounded-lg shadow-lg p-4 relative">
        <button
          onClick={onDismiss}
          className="absolute top-3 right-3 text-muted-foreground hover:text-foreground p-1 rounded-sm hover:bg-accent transition-colors"
        >
          <X size={16} />
        </button>
